...
import {
  deleteRentalInquiry,
  updateRentalInquiryLastContacted,
...
/**
 * PATCH /api/admin/rental-inquiries/[id] — update pipeline fields on a
 * rental inquiry. Mirrors the buyer + agent handlers one-to-one; parallel
 * route because the three tables are separate in the schema. See
 * inquiries/[id]/route for the field rules (status / notes /
 * last_contacted_at).
 */
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.status !== undefined) {
    ...
    latest = await updateRentalInquiryStatus(id, body.status as InquiryStatus, user.id);
    if (!latest) {
  ...
  if (body.notes !== undefined) {
    ...
    latest = await updateRentalInquiryNotes(id, body.notes as string | null);
    if (!latest) {
      return NextResponse.json({ error: "Rental inquiry not found." }, { status: 404 });
    }
  }
  ...
  if (body.last_contacted_at !== undefined) {
    ...
    latest = await updateRentalInquiryLastContacted(id, when);
    if (!latest) {
      return NextResponse.json({ error: "Rental inquiry not found." }, { status: 404 });
    }
  }
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const ok = await deleteRentalInquiry(id);
  if (!ok) {
    return NextResponse.json({ error: "Rental inquiry not found." }, { status: 404 });
  }
