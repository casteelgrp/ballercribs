...
function formatPublishedAt(d: Date | null): string {
  if (!d) return "";
  // Use UTC components so the stamp doesn't drift across TZs — same
  // approach used by the admin inquiry detail for DATE columns.
...
function BlogCard({
  ...
  return (
    <Link href={`/blog/${post.slug}`} className="group block">
      ...
      <div className="mt-4">
        <p className="text-[10px] uppercase tracking-widest text-accent">{categoryLabel}</p>
